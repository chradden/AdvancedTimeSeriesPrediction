...
# -*- coding: utf-8 -*-
"""
wind_power FORECASTING - ADVANCED TESTING (Option 2)
...
# SETUP
# =============================================================================
print_section("wind_power FORECASTING - ADVANCED MODELS TESTING (Option 2)")
print("\nModels to test:")
print("  1. Autoencoder (Anomaly Detection)")
print("  2. LightGBM Quantile (P10/P50/P90)")
print("  3. N-BEATS (if darts available)")

data_path = Path('data/raw/wind_power_2022-01-01_2024-12-31_hour.csv')
...
if (proc_dir / 'wind_onshore_train.csv').exists():
    print("Loading existing preprocessed data...")
    train_df = pd.read_csv(proc_dir / 'wind_onshore_train.csv', index_col=0)
    val_df = pd.read_csv(proc_dir / 'wind_onshore_val.csv', index_col=0)
    test_df = pd.read_csv(proc_dir / 'wind_onshore_test.csv', index_col=0)
    
    # Separate features and target
    feature_cols = [c for c in train_df.columns if c != 'wind_power']
    X_train = train_df[feature_cols].values
    y_train = train_df['wind_power'].values
    X_val = val_df[feature_cols].values
    y_val = val_df['wind_power'].values
    X_test = test_df[feature_cols].values
    y_test = test_df['wind_power'].values
    ...
    print("   Execute: python scripts/run_wind_onshore_extended_pipeline.py")
    exit(1)

# Load baseline Random Forest result for comparison
baseline_results = pd.read_csv(metrics_dir / 'wind_onshore_all_models_extended.csv')
rf_baseline = baseline_results[baseline_results['Model'] == 'Random Forest'].iloc[0]
print(f"\n📊 Baseline Random Forest: R²={rf_baseline['R²']:.4f}, RMSE={rf_baseline['RMSE']:.2f}")
...
try:
    ...
    # Actual wind_power with anomalies marked
    axes[1].plot(y_test, linewidth=1, alpha=0.7, color='black', label='Actual wind_power')
    ...
    axes[1].set_title('wind_power with Detected Anomalies', fontweight='bold', fontsize=14)
    axes[1].set_xlabel('Test Sample Index')
    axes[1].set_ylabel('wind_power (MW)')
    axes[1].legend()
    axes[1].grid(alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(figures_dir / 'wind_onshore_advanced_01_autoencoder_anomalies.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    with open(metrics_dir / 'wind_onshore_autoencoder_anomalies.json', 'w') as f:
        json.dump(anomaly_results, f, indent=2)
    
    print(f"\n✅ Autoencoder complete!")
    print(f"   Anomalies: {n_anomalies} detected ({anomaly_pct:.2f}%)")
    print(f"   Visualization saved: wind_onshore_advanced_01_autoencoder_anomalies.png")
...
ax.set_title('LightGBM Quantile Regression - Probabilistic wind_power Forecast (7 Days)', 
            fontweight='bold', fontsize=14)
ax.set_xlabel('Hours')
ax.set_ylabel('wind_power (MW)')
ax.legend(loc='best', fontsize=11)
ax.grid(alpha=0.3)

plt.tight_layout()
plt.savefig(figures_dir / 'wind_onshore_advanced_02_quantile_forecast.png', dpi=150, bbox_inches='tight')
plt.close()
...
print(f"   Visualization saved: wind_onshore_advanced_02_quantile_forecast.png")
...
with open(metrics_dir / 'wind_onshore_quantile_results.json', 'w') as f:
...
try:
    ...
    # Load raw wind_power data (N-BEATS is univariate)
    ...
    # Create TimeSeries
    series = TimeSeries.from_dataframe(df, value_cols='wind_power', freq='H')
    ...
    ax.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(figures_dir / 'wind_onshore_advanced_03_nbeats_forecast.png', dpi=150, bbox_inches='tight')
    plt.close()
    
    print(f"   Visualization saved: wind_onshore_advanced_03_nbeats_forecast.png")
    ...
    with open(metrics_dir / 'wind_onshore_nbeats_results.json', 'w') as f:
...
if advanced_results:
    ...
    print("="*80)
    print(f"LightGBM (Baseline):     R²={rf_baseline['R²']:.4f}  RMSE={rf_baseline['RMSE']:.2f}")
    for _, row in results_df.iterrows():
        delta_r2 = row['R²'] - rf_baseline['R²']
        symbol = "✅" if delta_r2 >= 0 else "⚠️"
        print(f"{row['Model']:25s} R²={row['R²']:.4f}  RMSE={row['RMSE']:.2f}  {symbol} Δ R²={delta_r2:+.4f}")
    
    # Save results
    results_df.to_csv(metrics_dir / 'wind_onshore_advanced_models_results.csv', index=False)
    print(f"\n✅ Results saved to wind_onshore_advanced_models_results.csv")
else:
...
summary = f"""
wind_power FORECASTING - ADVANCED MODELS (Option 2)
Execution Time: {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')}

BASELINE (for comparison):
  LightGBM: R²={rf_baseline['R²']:.4f}, RMSE={rf_baseline['RMSE']:.2f} MW

ADVANCED MODELS TESTED:

1. AUTOENCODER (Anomaly Detection):
   Purpose: Detect unusual wind_power patterns
   ...
   Use Case: Flag suspicious wind_power spikes for manual review
...
{'✅ N-BEATS R²=' + f"{result_nbeats['R²']:.4f}" if 'result_nbeats' in locals() else '⚠️ N-BEATS could not be tested'}
   → {'Performance vs LightGBM: ' + ('Better!' if 'result_nbeats' in locals() and result_nbeats['R²'] > rf_baseline['R²'] else 'Slightly lower (expected - no external features)') if 'result_nbeats' in locals() else 'Requires darts library'}

RECOMMENDATION:

For PRODUCTION:
  Primary:     LightGBM (R²={rf_baseline['R²']:.4f})
  Uncertainty: LightGBM Quantile (P10/P50/P90)
  Monitoring:  Autoencoder (anomaly detection)
  
Skip:
  N-BEATS:     {'Marginally worse than LightGBM' if 'result_nbeats' in locals() and result_nbeats['R²'] < rf_baseline['R²'] else 'Requires long training time'}
...
with open(metrics_dir / 'wind_onshore_ADVANCED_TESTING_SUMMARY.txt', 'w', encoding='utf-8') as f:
    f.write(summary)
